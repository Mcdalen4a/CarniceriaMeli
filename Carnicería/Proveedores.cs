...
namespace Carnicería
{
    public partial class Proveedores : Form
    {
        ...
        {
            string updateQuery = "UPDATE PROVEEDORES SET Telefono = @Telefono, Nombre = @Nombre, Apellido = @Apellido, Producto = @Producto WHERE ID = @ID";
            string insertQuery = "INSERT INTO PROVEEDORES (Telefono, Nombre, Apellido, Producto) VALUES (@Telefono, @Nombre, @Apellido, @Producto)";
                            ...
                            foreach (DataGridViewRow row in dataGridView1.Rows)
                            {
                                if (!row.IsNewRow)
                                {
                                    if (row.Cells["ID"].Value != null && !string.IsNullOrEmpty(row.Cells["ID"].Value.ToString()))
                                    {
                                        ...
                                        updateCommand.Parameters.AddWithValue("@Producto", row.Cells["Producto"].Value ?? (object)DBNull.Value);
                                    ...
                                    else
                                    {
                                        ...
                                        insertCommand.Parameters.AddWithValue("@Producto", row.Cells["Producto"].Value ?? (object)DBNull.Value);
